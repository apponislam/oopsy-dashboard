...
export function ContentModal({ isOpen, onClose, activeTab, editingItem, onSave }: ContentModalProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-gray-100 space-y-5 relative">
                ...
                <div className="space-y-1">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{editingItem ? `Edit ${isFaq ? "FAQ" : "Policy"}` : `Create New ${isFaq ? "FAQ" : "Policy"}`}</h3>
                </div>

                <form onSubmit={handleFormSubmit} className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold uppercase tracking-wider text-gray-700 block">{isFaq ? "Question" : "Document Title"}</label>
                        <Input placeholder={isFaq ? "e.g. How do I book a facility?" : "e.g. Terms of Service"} value={titleOrQuestion} onChange={(e) => setTitleOrQuestion(e.target.value)} className="rounded-xl" required />
                    </div>

                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold uppercase tracking-wider text-gray-700 block">Category</label>
                        <Input placeholder="e.g. Booking, Payment, Safety, Legal" value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-xl" required />
                    </div>

                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold uppercase tracking-wider text-gray-700 block">{isFaq ? "Answer" : "Policy Content"}</label>
                        <textarea
